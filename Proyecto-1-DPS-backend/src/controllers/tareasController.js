...
const createTarea = (req, res) => {
  const { titulo, descripcion, proyecto_id, asignado_a, prioridad, fecha_vencimiento } = req.body; //cambio de nombres de variables para que hagan match con la bd, agregue la prioridad
  ...
  Usuario.getById(asignado_a, (err, usuario) => {
    ...
    const nuevaTarea = { titulo, descripcion, proyecto_id, asignado_a, prioridad, fecha_vencimiento }; //removi estado de aca para que se cree como pendiente por default
...
const updateTarea = (req, res) => {
  const { id } = req.params;
  const { titulo, descripcion, estado, prioridad, asignado_a, fecha_vencimiento, proyecto_id } = req.body;
  const actualizacion = { titulo, descripcion, asignado_a, estado, prioridad, proyecto_id, fecha_vencimiento };
